...
export function generateA5BillReceiptHTML(data) {
  ...
  const actualTotalPaid = totalPaid > 0 ? totalPaid : advanceReceived + currentPayment;
  const actualBalanceDue = netAmount - actualTotalPaid;
  const amountInWords = numberToWords(actualTotalPaid > 0 ? actualTotalPaid : netAmount);
  const reportPin = generateReportPin(bookingNo || invoiceNo);

  const isFullyPaid = actualBalanceDue <= 0;
  const isUnpaid = actualTotalPaid <= 0;
...
<head>
  <meta charset="utf-8" />
  <title>Final Tax Invoice - ${invoiceNo}</title>
  <style>
    @page {
      size: A5 landscape;
      margin: 4mm 6mm;
    ...
    * {
    ...
    /* Outer Border Box matching classic rptbill_receipts structure */
    .receipt-container {
...
<body>
  ...
  <div class="receipt-container">
    ...
    <table class="meta-table">
      <tr>
        <td class="meta-label">Bill / Inv No:</td>
        <td class="meta-val" style="color: #070a61; font-size: 12px; font-family: monospace;">${invoiceNo}</td>
      ...
      <tbody>
        ${
          ...
          <tr>
            ...
            <td style="font-weight: 600;">${t.test_name || t.name || t.Descr || 'Diagnostic Test'}</td>
    ...
    <!-- Summary & Financial Audit Block (rptbill_receipts specification) -->
    <table class="summary-wrapper">
      <tr>
        <!-- Left Column: Words & Security PIN -->
        <td class="summary-left">
          <div class="words-box">
            <strong>Received with thanks:</strong> ${amountInWords}
          </div>
          ...
          <div style="margin-top: 6px; font-size: 9.5px; color: #475569;">
            * Thank you for choosing Santoshpur Diagnostic Centre.
          </div>
        </td>
        ...
        <td class="summary-right">
          <table class="calc-table">
            <tr>
              ...
              <td class="calc-val">₹ ${parseFloat(totalAmount || netAmount).toFixed(2)}</td>
            </tr>
            <tr>
            ...
            <tr class="calc-highlight">
              <td class="calc-label" style="color: #070a61; font-weight: 800;">Amount Payable:</td>
              <td class="calc-val" style="color: #070a61; font-weight: 900; font-size: 11px;">₹ ${parseFloat(netAmount).toFixed(2)}</td>
            </tr>
            <tr>
              <td class="calc-label" style="color: #475569;">Advance Received:</td>
              <td class="calc-val">₹ ${parseFloat(advanceReceived).toFixed(2)}</td>
            </tr>
            ${
              currentPayment > 0
                ? `
              <tr>
                <td class="calc-label" style="color: #15803d; font-weight: 700;">Current Payment:</td>
                <td class="calc-val" style="color: #15803d; font-weight: 800;">₹ ${parseFloat(currentPayment).toFixed(2)} (${paymentMethod})</td>
              </tr>
            `
                : ''
            }
            <tr style="border-top: 1px solid #cbd5e1;">
              <td class="calc-label" style="color: #15803d; font-weight: 800;">Total Paid to Date:</td>
              <td class="calc-val" style="color: #15803d; font-weight: 900;">₹ ${parseFloat(actualTotalPaid).toFixed(2)}</td>
            </tr>
            <tr>
    ...
    <div class="footer-section">
      <div>
        ...
        <div class="footer-tagline">"Your Health, Our Care" • Report Delivery 7 P.M - 8 P.M.</div>
      </div>
